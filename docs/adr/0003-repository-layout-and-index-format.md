...
```yaml
# AUTO-GENERATED by adr index - DO NOT EDIT
...
- Running `adr index` regenerates the file completely
- The index should be committed to version control for agent access
- CI should run `adr index --check` to verify index is up-to-date
...
**Negative:**
- Index can become stale (mitigated by CI check with `adr index --check`)
...
- Run `adr index` after creating/modifying ADRs
- Use `adr list` to query ADRs (uses index if available)
